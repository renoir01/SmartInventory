                    ...
                    try:
                        if table_name.lower() == 'product':
                            ...
                            if total_count > 0:
                                ...
                                if total_count > 5:
                                    print(f"    ... and {total_count - 5} more products")
                        
                        elif table_name.lower() == 'sale':
                            cursor.execute(f"SELECT * FROM {table_name} LIMIT 1")
                            columns = [description[0] for description in cursor.description]
                            print(f"  Sale table columns: {columns}")
                            ...
                            cursor.execute(f"SELECT * FROM {table_name} LIMIT 1")
                            columns = [description[0] for description in cursor.description]
                            print(f"  Monthly profit table columns: {columns}")
...
def main():
    # Check the downloaded database file
    db_file = r"c:\Users\user\Downloads\new_inventory.db.backup_20250408_185759"
    
    print("Checking Downloaded Database Backup")
    print("===================================")
